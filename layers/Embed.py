...
class ReversePositionalEmbedding(nn.Module):
    def __init__(self, d_model, max_len=5000):
        super(ReversePositionalEmbedding, self).__init__()
        # Compute the positional encodings once in log space.
        pe = torch.zeros(max_len, d_model).float()
        pe.require_grad = False

        # Generate positions in reverse order
        position = torch.arange(max_len - 1, -1, -1).float().unsqueeze(1)
        div_term = (
    ...
    def forward(self, x):
        # Retrieve the reverse positional encodings based on the input sequence length
        return self.pe[:, : x.size(1)]
...
class DataEmbedding_onlyposRPE(nn.Module):
    def __init__(self, c_in, d_model, embed_type="fixed", freq="h", dropout=0.1):
        super(DataEmbedding_onlyposRPE, self).__init__()

        self.value_embedding = TokenEmbedding(c_in=c_in, d_model=d_model)
        self.position_embeddingRPE = ReversePositionalEmbedding(d_model=d_model)
        self.dropout = nn.Dropout(p=dropout)

    def forward(self, x, x_mark):
        x = self.position_embeddingRPE(x) + self.value_embedding(x)
        return self.dropout(x)
...
class TcEmbedding(nn.Module):
    def __init__(self, max_len, d_model):
        super(TcEmbedding, self).__init__()
    ...
    def forward(self, tc_sequence):
        tc_embed = self.embedding(tc_sequence)
        # return tc_embed * math.sqrt(self.d_model)
        return self.fc(tc_embed)
...
class DataEmbedding_position_Tc(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', dropout=0.1, tcVec=None):
        ...
        self.dropout = nn.Dropout(p=dropout)
        self.tcVec = tcVec
        self.tc_embedding = TcEmbedding(max_len=np.size(tcVec,0), d_model=d_model)
